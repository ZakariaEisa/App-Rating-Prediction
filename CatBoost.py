...
def main():
    ...
    print("Train MAE(catBoost Model):", mean_absolute_error(y_train, y_pred_train))
    print("Validation MAE (catBoost Model):", mean_absolute_error(y_val, y_pred_val))

    y_pred_test = blend(cat.predict(X_test))
    submission['Y'] = y_pred_test
    submission.to_csv('0FinalP (catBoost Model).csv', index=False)
    print("✅ Saved 0FinalP (catBoost Model).csv")
