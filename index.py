...
def main():
    ...
    print("Train MAE (stack model):", mean_absolute_error(y_train, y_pred_train))
    print("Validation MAE (stack model):", mean_absolute_error(y_val, y_pred_val))

    y_pred_test = blend(stack.predict(X_test))
    submission['Y'] = y_pred_test
    submission.to_csv('0FinalP(stack model).csv', index=False)
    print("✅ Saved 0FinalP.csv")
