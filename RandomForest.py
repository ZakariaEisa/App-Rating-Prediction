...
def main():
    ...
    print("Train MAE (Random Forest):", mean_absolute_error(y_train, y_pred_train))
    print("Validation MAE (Random Forest):", mean_absolute_error(y_val, y_pred_val))

    y_pred_test = blend(rf.predict(X_test))
    submission['Y'] = y_pred_test
    submission.to_csv('0FinalP (RandomForest Model).csv', index=False)
    print("✅ Saved 0FinalP (RandomForest Model).csv")
